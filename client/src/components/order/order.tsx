...
export const Order: React.FC<TOrder> = ({order, userId, orderId, created_at, completed}): JSX.Element => {
    ...
    const handleRemove = (orderId: string) => {
        const answer = window.confirm(`Вы удаляете заказ № ${orderId.slice(-4)}. Продолжить?`)
        if (answer) {
    ...
    return (
        <div className={styles.order_container}>
            ...
            <p>created_at: <span className={styles.order_title}>{new Date(created_at).toLocaleString()}</span></p>
